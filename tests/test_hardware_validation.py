...
class DummyBackend:
    def __init__(self, operation_names, num_qubits=7, name="dummy_backend"):
        self.target = DummyTarget(operation_names)
        self.num_qubits = num_qubits
        self.name = name


def test_backend_support_helpers():
...
def test_validate_backend_rejects_missing_if_else_for_dynamic_mode():
...
def test_validate_backend_accepts_deferred_mode_without_if_else():
    backend = DummyBackend({"delay", "measure"}, num_qubits=7)
    validate_backend_for_experiment(
        backend,
        n_required_qubits=3,
