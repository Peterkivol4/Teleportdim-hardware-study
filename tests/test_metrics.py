...
def test_fill_ratio_distinguishes_d3_and_d4_on_same_two_qubits():
    assert np.isclose(fill_ratio(3), 0.75)
    assert np.isclose(fill_ratio(4), 1.0)


def test_leakage_probability_detects_population_outside_codespace():
...
def test_in_subspace_fidelity_conditions_on_codespace_population():
...
def test_average_gate_fidelity_formula():
